# =============================================================================
# TUTORIAL 2: Inferencia de Redes Regulatorias con pySCENIC — protocolo 2021
...
#          (PBMC3k, las MISMAS células) -> runs/scanpy_*/pbmc3k_for_scenic.loom
# Salida:  auc_matrix.csv + pyscenic_output.loom
#
# Requiere el entorno conda `scenic-2021` (ver environment.yml). Con ese
...
def _find_latest_scanpy_loom() -> Path | None:
    ...
    for run in sorted(Path("runs").glob("scanpy_*"), reverse=True):
        loom = run / "pbmc3k_for_scenic.loom"
        if loom.exists():
            return loom
    return None
...
def write_report(
    ...
    lines += [
        "# Reporte Técnico — SCENIC pipeline (PBMC3k, protocolo 2021)",
        "",
...
try:
    ...
    if LOOM_PATH is None:
        raise FileNotFoundError(
            "No se encontró runs/scanpy_*/pbmc3k_for_scenic.loom. "
    ...
    # ==========================================================================
    # PASO 1: CARGAR MATRIZ (solo para métricas/AUCell posterior)
    # ==========================================================================
