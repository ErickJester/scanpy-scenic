...
# combina la matriz AUC de regulones con el AnnData de clustering (mismas
# células), y exporta un loom integrado listo para SCope.
# =============================================================================
...
def write_report(
    adata=None, auc_matrix=None, scanpy_dir=None, scenic_dir=None,
    n_matched=0, error: str = None,
) -> None:
    ...
    if adata is not None:
        ...
        match_ok   = n_matched > 0

        lines += [
            f"| AnnData cargado | Sí | Sí | ✓ |",
            f"| Células con AUC (match) | > 0 | {n_matched:,} | {'✓' if match_ok else '✗ SIN MATCH'} |",
...
try:
    ...
    if scanpy_dir is None:
        raise FileNotFoundError("No se encontró runs/scanpy_*. Ejecuta primero 01_scanpy_clustering.py")
    if scenic_dir is None:
        raise FileNotFoundError("No se encontró runs/scenic_*. Ejecuta primero 02_scenic_pipeline.py")
    ...
    if n_matched == 0:
        ...
        auc_aligned = pd.DataFrame(np.nan, index=adata.obs_names, columns=auc_matrix.columns)
    else:
    ...
    if n_matched > 0 and "X_umap" in adata.obsm:
        ...
        sc.pl.umap(
            adata, color=top_obs_names, ncols=2,
            title=[f"Regulón: {r}" for r in top_regs.index],
            show=False, save="_regulon_activity.png",
        )
        ...
        if "cell_type" in adata.obs.columns:
            ...
            sns.heatmap(mean_by_ct, cmap="viridis", annot=False, ax=ax, linewidths=0.5)
            ax.set_title("Actividad media de regulones por tipo celular")
            ax.set_ylabel("Tipo celular")
            ax.set_xlabel("Regulón")
            plt.tight_layout()
            fig.savefig(str(FIG_DIR / "heatmap_regulons_by_celltype.png"), dpi=80, bbox_inches="tight")
            plt.close(fig)
            rlog("- Heatmap regulones × tipo celular guardado")

            sc.pl.umap(
                adata, color=["cell_type", top_obs_names[0]], ncols=2,
                title=["Tipo celular", f"Top regulón: {top_regs.index[0]}"],
                show=False, save="_celltype_vs_regulon.png",
            )
    ...
    for col in adata.obs.columns:
        if col.startswith("scenic_"):
            ...
            col_attrs[col] = np.array(np.nan_to_num(vals, nan=0.0), dtype=np.float32)
    ...
    write_report(
        adata=adata, auc_matrix=auc_matrix, scanpy_dir=scanpy_dir,
        scenic_dir=scenic_dir, n_matched=n_matched, error=error,
    )
    total = time.time() - _start_time
    print(f"\n{'='*60}")
